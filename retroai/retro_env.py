...
class RetroEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        scenario=None,
        info=None,
        use_restricted_actions=retroai.enums.Actions.FILTERED,
        record=False,
        players=1,
        ...
        core = retro_get_system_info(self.system)
        ...
        try:
            ...
            ), "Failed to load info (%s) or scenario (%s)" % (info_path, scenario_path)
        except Exception:
            del self.em
            raise
        ...
        if use_restricted_actions == retroai.enums.Actions.DISCRETE:
            ...
            self.action_space = gymnasium.spaces.MultiBinary(self.num_buttons * players)
    ...
    def action_to_array(self, a):
        ...
        for p in range(self.players):
            ...
            if self.use_restricted_actions == retroai.enums.Actions.DISCRETE:
            ...
            elif self.use_restricted_actions == retroai.enums.Actions.MULTI_DISCRETE:
                ...
                for i in range(len(ap)):
                    action |= int(ap[i]) << i
                if self.use_restricted_actions == retroai.enums.Actions.FILTERED:
