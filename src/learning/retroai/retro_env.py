...
try:
    ...
    print(
        "OpenAI modules must be built first. See README.md for more information."
    )
    sys.exit(1)
...
class RetroEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        scenario=None,
        info=None,
        use_restricted_actions: retroai.enums.Actions = retroai.enums.Actions.FILTERED,
        record: bool = False,
        players: int = 1,
        inttype: retro.data.Integrations = retro.data.Integrations.STABLE,
        obs_type: retroai.enums.Observations = retroai.enums.Observations.IMAGE,
    ) -> None:
        ...
        self.viewer = None
        self.gamename = game
        self.statename: Optional[str] = str(state)
        ...
        if state == retroai.enums.State.NONE:
            self.statename = None
        elif state == retroai.enums.State.DEFAULT:
            self.statename = None
            try:
        ...
        if scenario.endswith(".json"):
            ...
            scenario_path = retro.data.get_file_path(
                game, scenario + ".json", inttype
            )

        self.system = retro_get_romfile_system(rom_path)
        ...
        gc.collect()
        self.em = RetroEmulator(rom_path)
        self.em.configure_data(self.data)
        self.em.step()

        core: dict[str, Any] = retro_get_system_info(self.system)
        ...
        try:
            ...
            ), "Failed to load info (%s) or scenario (%s)" % (
                info_path,
                scenario_path,
            )
        except Exception:
        ...
        if use_restricted_actions == retroai.enums.Actions.DISCRETE:
            combos = 1
            for combo in self.button_combos:
                combos *= len(combo)
            self.action_space = gymnasium.spaces.Discrete(combos**players)
        elif use_restricted_actions == retroai.enums.Actions.MULTI_DISCRETE:
            self.action_space = gymnasium.spaces.MultiDiscrete(
                [len(combos) for combos in self.button_combos] * players
            )
        else:
            self.action_space = gymnasium.spaces.MultiBinary(
                self.num_buttons * players
            )

        kwargs: dict[str, Any] = {}
        if True:  # or gym_version >= (0, 9, 6):
            kwargs["dtype"] = np.uint8

        shape: tuple[int, ...]
        if self._obs_type == retroai.enums.Observations.RAM:
            shape = self.get_ram().shape
        else:
            img = [self.get_screen(p) for p in range(players)]
            shape = img[0].shape
        self.observation_space = gymnasium.spaces.Box(
            low=0, high=255, shape=shape, **kwargs
        )
    ...
    def _update_obs(self):
        if self._obs_type == retroai.enums.Observations.RAM:
            ...
            return self.ram
        elif self._obs_type == retroai.enums.Observations.IMAGE:
    ...
    def action_to_array(self, a):
        ...
        for p in range(self.players):
            ...
            if self.use_restricted_actions == retroai.enums.Actions.DISCRETE:
                ...
                self.use_restricted_actions
                == retroai.enums.Actions.MULTI_DISCRETE
            ):
    ...
    def reset(self):
        ...
        self.em.step()
        if self.movie_path is not None and self.statename is not None:
            rel_statename = os.path.splitext(os.path.basename(self.statename))[
                0
            ]
            self.record_movie(
                os.path.join(
    ...
    def seed(self, seed=None):
        self.np_random, seed1 = gymnasium.utils.seeding.np_random(seed)
        seed2 = gymnasium.utils.seeding.np_random(seed)
        return [seed1, seed2]
    ...
    def get_screen(self, player: int = 0):
    ...
    def load_state(
        self, statename, inttype=retro.data.Integrations.DEFAULT
    ) -> None:
    ...
    def compute_step(self) -> tuple[list[float], bool, dict[str, Any]]:
        reward: list[float]
        if self.players > 1:
...
def retro_make(
    game: str,
    state: retroai.enums.State = retroai.enums.State.DEFAULT,
    inttype: retro.data.Integrations = retro.data.Integrations.DEFAULT,
    **kwargs
) -> RetroEnv:
    """
